...
def score_token_world(user_id):
    ...
    if (skill_id == "skill_ilith_ivy" or skill_id == "skill_hikari_vanessa") and d(
        "is_skill_sealed"
    ) == "false":
...
def score_token_course(user_id):
    with Connect() as c:
        use_course_skip_purchase = (
            request.args.get("use_course_skip_purchase", "false") == "true"
        )
...
def song_score_post(user_id):
    with Connect() as c:
        ...
        x.set_score(
            request.form["score"],
            request.form["shiny_perfect_count"],
            request.form["perfect_count"],
            request.form["near_count"],
            request.form["miss_count"],
            request.form["health"],
            request.form["modifier"],
            int(time() * 1000),
            request.form["clear_type"],
        )
...
def song_score_top(user_id):
    with Connect() as c:
        ...
        rank_list.song.set_chart(
            request.args.get("song_id"), request.args.get("difficulty")
        )
