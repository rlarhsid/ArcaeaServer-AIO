...
class RankList:
    ...
    def select_top(self) -> None:
        ...
        if self.limit >= 0:
            self.c.execute(
                """select * from best_score where song_id = :song_id and difficulty = :difficulty order by score DESC, time_played DESC limit :limit""",
                {
            ...
            self.c.execute(
                """select * from best_score where song_id = :song_id and difficulty = :difficulty order by score DESC, time_played DESC""",
        ...
        user_info_list = Sql(self.c).select(
            "user",
            [
                "user_id",
                "name",
            ...
            Query().from_args({"user_id": [i[0] for i in x]}),
        )
    ...
    def select_friend(self, user=None, limit=Constant.MAX_FRIEND_COUNT) -> None:
        ...
        self.c.execute(
            f"""select * from best_score where user_id in ({','.join(['?'] * len(user_ids))}) and song_id = ? and difficulty = ? order by score DESC, time_played DESC limit ?""",
            user_ids + [self.song.song_id, self.song.difficulty, self.limit],
        )
        x = self.c.fetchall()
        if not x:
            return None
    ...
    def get_my_rank_parameter(
        ...
        amount: int,
        all_limit: int = 20,
        max_local_position: int = Constant.MY_RANK_MAX_LOCAL_POSITION,
        ...
        if my_rank <= max_local_position:  # 排名在前面，前方人数不足
            ...
            amount - my_rank < all_limit - max_local_position
        ):  # 后方人数不足，显示排名
            sql_offset = amount - all_limit
        elif (
            max_local_position
            <= my_rank
            <= max_global_position - all_limit + max_local_position - 1
        ):  # 前方人数足够，显示排名
    ...
    def select_me(self, user=None) -> None:
        ...
        if user:
            self.user = user
        self.c.execute(
            """select score, time_played from best_score where user_id = :user_id and song_id = :song_id and difficulty = :difficulty""",
            {
        ...
        self.c.execute(
            """select count(*) from best_score where song_id = :song_id and difficulty = :difficulty and ( score > :score or (score = :score and time_played > :time_played) )""",
            {
                "user_id": self.user.user_id,
                "song_id": self.song.song_id,
                "difficulty": self.song.difficulty,
        ...
        self.c.execute(
            """select count(*) from best_score where song_id=:a and difficulty=:b""",
        ...
        self.c.execute(
            """select * from best_score where song_id = :song_id and difficulty = :difficulty order by score DESC, time_played DESC limit :limit offset :offset""",
            {
                "song_id": self.song.song_id,
                "difficulty": self.song.difficulty,
                "limit": sql_limit,
