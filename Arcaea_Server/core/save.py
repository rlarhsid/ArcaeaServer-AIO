...
class SaveData:
    ...
    def to_dict(self):
        return {
            ...
            "devicemodelname": {"val": self.devicemodelname_data},
            "installid": {"val": self.installid_data},
            "unlocklist": {"": self.unlocklist_data},
            "clearedsongs": {"": self.clearedsongs_data},
            "clearlamps": {"": self.clearlamps_data},
            "scores": {"": self.scores_data},
            "version": {"val": 1},
            "createdAt": self.createdAt,
            "finalestate": {"val": self.finalestate_data},
        }
    ...
    def select_scores(self, user) -> None:
        ...
        self.user = user
        self.c.execute(
            """select scores_data, clearlamps_data from user_save where user_id=:a""",
            {"a": user.user_id},
        )
        x = self.c.fetchone()
        if not x:
    ...
    def select_all(self, user) -> None:
        ...
        self.user = user
        self.c.execute(
            """select * from user_save where user_id=:a""", {"a": user.user_id}
        )
        x = self.c.fetchone()
        if x:
        ...
        if Config.SAVE_FULL_UNLOCK:
            ...
            self.finalestate_data = "|".join(
                ["0", "100"]
                + [
                    str(x[0]) if i in [64, 65, 66, 71] else str(x[1])
                    for i, x in enumerate(Constant.FINALE_SWITCH)
                ]
                + ["1337"]
            )
    ...
    def update_all(self, user) -> None:
        ...
        self.c.execute(
            """insert or replace into user_save values(:a,:b,:c,:d,:e,:f,:g,:h,:i,:j)""",
            {
                "a": user.user_id,
                "b": json.dumps({"": self.scores_data}),
                "c": json.dumps({"": self.clearlamps_data}),
                "d": json.dumps({"": self.clearedsongs_data}),
                "e": json.dumps({"": self.unlocklist_data}),
                "f": json.dumps({"val": self.installid_data}),
                "g": json.dumps({"val": self.devicemodelname_data}),
                "h": json.dumps({"": self.story_data}),
                "i": self.createdAt,
                "j": self.finalestate_data,
            },
        )
