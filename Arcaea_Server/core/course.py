...
class CourseChart(Chart):
    ...
    def insert_course_chart(self, course_id: str, song_index: int) -> None:
        self.c.execute(
            "insert into course_chart values (?,?,?,?,?)",
...
class Course:
    ...
    def from_dict(self, d: dict) -> "Course":
        ...
        self.charts = [CourseChart(self.c).from_dict(x) for x in d.get("songs", [])]
        self.items = [ItemFactory.from_str(x, self.c) for x in d.get("rewards", [])]
    ...
    def select_course_chart(self) -> None:
        ...
        for i in self.c.fetchall():
            self.charts[i[4]] = CourseChart(self.c).from_dict(
                {"id": i[1], "difficulty": i[2], "flag_as_hidden": i[3] == 1}
            )

    def select_course_requirement(self) -> None:
        self.c.execute(
            """select required_id from course_requirement where course_id = ?""",
            (self.course_id,),
        )
    ...
    def insert_course(self) -> None:
        self.c.execute(
            ...
            (
                ...
                self.dan_name,
                self.style,
                self.gauge_requirement,
                self.flag_as_hidden_when_requirements_not_met,
                self.can_start,
            ),
        )
    ...
    def insert_course_item(self) -> None:
        for i in self.items:
            self.c.execute(
                """insert into course_item values (?,?,?,?)""",
    ...
    def insert_course_requirement(self) -> None:
        for i in self.requirements:
            self.c.execute(
                """insert into course_requirement values (?,?)""", (self.course_id, i)
            )
...
class UserCourse(Course):
    ...
    def insert_or_update_user_course(self) -> None:
        self.c.execute(
            """insert or replace into user_course values (?,?,?,?)""",
...
class UserCourseList:
    ...
    def select_all(self) -> None:
        ...
        self.courses = [
            UserCourse(self.c, self.user).from_list(x) for x in self.c.fetchall()
        ]
...
class CoursePlay(UserCourse):
    ...
    def update(self) -> None:
        ...
        if Score.get_song_state(self.clear_type) > Score.get_song_state(
            self.user_play.clear_type
        ):
