...
class Redeem(CollectionItemMixin):
    ...
    def to_dict(self, has_items: bool = True) -> dict:
        r = {"code": self.code, "type": self.redeem_type}
        if has_items:
    ...
    def from_dict(self, d: dict) -> "Redeem":
        ...
        self.items = [ItemFactory.from_dict(i, c=self.c) for i in d.get("items", [])]
        return self
    ...
    def select(self, code: str = None) -> "Redeem":
        if code:
            self.code = code
        self.c.execute("""select * from redeem where code=:a""", {"a": self.code})
        x = self.c.fetchone()
        if x is None:
    ...
    def select_items(self) -> None:
        self.c.execute("""select * from redeem_item where code=:a""", {"a": self.code})
        self.items = [
            ItemFactory.from_dict(
                {"item_id": i[1], "type": i[2], "amount": i[3] if i[3] else 1}, self.c
            )
    ...
    def insert(self) -> None:
        self.c.execute(
            """insert into redeem values(?,?)""", (self.code, self.redeem_type)
        )
    ...
    def delete_redeem_item(self) -> None:
        self.c.execute("""delete from redeem_item where code=?""", (self.code,))
    ...
    def update(self) -> None:
        self.c.execute(
            """update redeem set type=? where code=?""", (self.redeem_type, self.code)
        )
...
class UserRedeem(Redeem):
    ...
    def is_available(self) -> bool:
        ...
        if self.redeem_type == 0:
            # 一次性
            self.c.execute(
                """select exists(select * from user_redeem where code=:a)""",
                {"a": self.code},
            )
            ...
            # 每个玩家一次
            self.c.execute(
                """select exists(select * from user_redeem where code=:a and user_id=:b)""",
    ...
    def insert_user_redeem(self) -> None:
        self.c.execute(
            """insert into user_redeem values(:b,:a)""",
            {"a": self.code, "b": self.user.user_id},
        )
    ...
    def claim_user_redeem(self, code: str = None) -> None:
        ...
        if not self.is_available:
            if self.redeem_type == 0:
                raise RedeemUnavailable("The redeem `%s` is unavailable." % self.code)
