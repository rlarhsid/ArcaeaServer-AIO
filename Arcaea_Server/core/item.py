...
class Item:
    ...
    def to_dict(self, has_is_available: bool = False, has_amount: bool = True) -> dict:
        r = {"id": self.item_id, "type": self.item_type}
        if has_amount:
    ...
    def insert(self, ignore: bool = False):
        sql = (
            ...
            if not ignore
            else """insert or ignore into item values(?,?,?)"""
        )
    ...
    def select(self):
        ...
        if not x:
            raise NoData(
                f"No such item `{self.item_type}`: `{self.item_id}`",
                api_error_code=-121,
            )
...
class NormalItem(UserItem):
    ...
    def user_claim_item(self, user):
        ...
        self.c.execute(
            """select exists(select * from user_item where user_id=? and item_id=? and type=?)""",
            (self.user.user_id, self.item_id, self.item_type),
        )
        if self.c.fetchone() == (0,):
            self.c.execute(
                """insert into user_item values(:a,:b,:c,1)""",
    ...
    def user_claim_item(self, user):
        ...
        if x:
            ...
            if self.amount < 0:  # 添加数量错误
                raise InputError("The amount of `%s` is wrong." % self.item_id)
            self.c.execute(
...
class ItemCore(PositiveItem):
    ...
    def to_dict(
        self,
        has_is_available: bool = False,
        has_amount: bool = True,
        character_format: bool = False,
    ) -> dict:
        if character_format:
            # 搭档的core是特殊格式的
...
class ItemCharacter(UserItem):
    ...
    def user_claim_item(self, user):
        ...
        self.c.execute(
            """select exists(select * from user_char where user_id=? and character_id=?)""",
            (user.user_id, self.item_id),
        )
        if self.c.fetchone() == (0,):
            self.c.execute(
...
class Memory(UserItem):
    ...
    def user_claim_item(self, user):
        self.c.execute("""select ticket from user where user_id=?""", (user.user_id,))
        x = self.c.fetchone()
        if x is not None:
            self.c.execute(
...
class UserItemList:
    ...
    def select_from_type(self, item_type: str) -> "UserItemList":
        ...
        """
        if (
            Config.WORLD_SONG_FULL_UNLOCK
            and item_type == "world_song"
            or Config.WORLD_SCENERY_FULL_UNLOCK
            and item_type == "world_unlock"
        ):
            self.c.execute("""select item_id from item where type=?""", (item_type,))
        else:
            self.c.execute(
                """select item_id, amount from user_item where type = :a and user_id = :b""",
...
class CollectionItemMixin:
    ...
    def add_items(self, items: "list[Item]") -> None:
        collection_id: "str" = getattr(self, self.collection_item_const["id_name"])
        ...
        for i in items:
            ...
            if i in collection_items:
                raise DataExist(
                    f'Item `{i.item_type}`: `{i.item_id}` already exists in {self.collection_item_const["name"]} `{collection_id}`',
                    api_error_code=-123,
                )
        self.c.executemany(
            f"""insert into {self.collection_item_const["table_name"]} values (?, ?, ?, ?)""",
    ...
    def remove_items(self, items: "list[Item]") -> None:
        ...
        for i in items:
            if i not in collection_items:
                raise NoData(
                    f'No such item `{i.item_type}`: `{i.item_id}` in {self.collection_item_const["name"]} `{collection_id}`',
                    api_error_code=-124,
                )
        self.c.executemany(
            f"""delete from {self.collection_item_const["table_name"]} where {self.collection_item_const["table_primary_key"]}=? and item_id=? and type=?""",
    ...
    def update_items(self, items: "list[Item]") -> None:
        ...
        self.c.executemany(
            f"""update {self.collection_item_const["table_name"]} set amount=? where {self.collection_item_const["table_primary_key"]}=? and item_id=? and type=?""",
