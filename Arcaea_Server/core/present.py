...
class Present(CollectionItemMixin):
    ...
    def select_exists(self) -> bool:
        ...
        """
        self.c.execute(
            """select exists(select * from present where present_id=?)""",
            (self.present_id,),
        )
    ...
    def insert_items(self) -> None:
        for i in self.items:
            ...
            self.c.execute(
                """insert or ignore into present_item values(?,?,?,?)""",
                (self.present_id, i.item_id, i.item_type, i.amount),
            )

    def insert(self) -> None:
        self.c.execute(
            """insert into present values(?,?,?)""",
    ...
    def delete(self) -> None:
        self.c.execute("""delete from present where present_id=?""", (self.present_id,))
    ...
    def update(self) -> None:
        self.c.execute(
            """update present set expire_ts=?, description=? where present_id=?""",
...
class UserPresent(Present):
    ...
    def delete_user_present(self) -> None:
        ...
        """
        self.c.execute(
            """delete from user_present where user_id=:a and present_id=:b""",
    ...
    def claim_user_present(self, present_id: str = None, user=None) -> None:
        ...
        if self.c.fetchone() == (0,):
            raise NoData(
                "The present `%s` for the user `%s` does not exist."
...
class UserPresentList:
    ...
    def select_user_presents(self) -> None:
        ...
        """
        self.c.execute(
            """select * from present where present_id in (select present_id from user_present where user_id=:a)""",
            {"a": self.user.user_id},
        )
        x = self.c.fetchall()
        self.presents = []
