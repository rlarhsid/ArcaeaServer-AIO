...
class SonglistParser:
    ...
    def get_user_unlocks(user) -> set:
        ...
        if Constant.SINGLE_PACK_NAME in x.pack_info:
            r.update(x.pack_info[Constant.SINGLE_PACK_NAME] & set(user.singles))
        r.update(
            set(
                i if i[-1] != "3" else i[:-1]
...
class UserDownload:
    ...
    limiter = ArcLimiter(str(Constant.DOWNLOAD_TIMES_LIMIT) + "/day", "download")
    ...
    def select_for_check(self) -> None:
        ...
        self.c_m.execute(
            """select user_id, time from download_token where song_id=? and file_name=? and token = ? limit 1;""",
    ...
    def insert_download_token(self) -> None:
        """将数据插入数据库，让这个下载链接可用"""
        self.c_m.execute(
            """insert or replace into download_token values(:a,:b,:c,:d,:e)""",
            {
                "a": self.user.user_id,
                "b": self.song_id,
                "c": self.file_name,
    ...
    def url(self) -> str:
        ...
        return url_for(
            "download",
            file_path=f"{self.song_id}/{self.file_name}",
            t=self.token,
            _external=True,
        )
...
class DownloadList(UserDownload):
    ...
    def clear_download_token(self) -> None:
        """清除过期下载链接"""
        self.c_m.execute(
            """delete from download_token where time<?""",
    ...
    def insert_download_tokens(self) -> None:
        ...
        self.c_m.executemany(
            """insert or replace into download_token values(?,?,?,?,?)""",
            [
                (self.user.user_id, x.song_id, x.file_name, x.token, x.token_time)
        ...
        for i in self.get_one_song_file_names(song_id):
            ...
            if i == "base.ogg":
                ...
                if self.url_flag:
                    ...
                    re["additional_files"].append({"checksum": x.hash, "file_name": i})
    ...
    def get_all_song_ids() -> list:
        """获取全歌曲文件夹列表，有lru缓存"""
        return [
            i.name for i in os.scandir(Constant.SONG_FILE_FOLDER_PATH) if i.is_dir()
        ]
    ...
    def add_songs(self, song_ids: list = None) -> None:
        ...
        if not self.song_ids:
            ...
            if Config.DOWNLOAD_FORBID_WHEN_NO_ITEM and SonglistParser.has_songlist:
                # 没有歌曲时不允许下载
                self.song_ids = list(
                    SonglistParser.get_user_unlocks(self.user) & set(self.song_ids)
                )
