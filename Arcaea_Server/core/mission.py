...
class Mission:
    ...
    def user_claim_mission(self, user):
        ...
        self.c.execute(
            """insert or replace into user_mission (user_id, mission_id, status) values (?, ?, 4)""",
    ...
    def user_clear_mission(self, user):
        ...
        self.c.execute(
            """insert or replace into user_mission (user_id, mission_id, status) values (?, ?, 2)""",
            (self.user.user_id, self.mission_id),
        )
        self._status = 2
...
class UserMissionList:
    ...
    def select_all(self):
        self.missions = []
        self.c.execute(
            """select mission_id, status from user_mission where user_id=?""",
            (self.user.user_id,),
        )
