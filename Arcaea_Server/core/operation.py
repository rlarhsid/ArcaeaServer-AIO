...
class RefreshAllScoreRating(BaseOperation):
    ...
    def run(self):
        ...
        with Connect() as c:
            ...
            c.execute(
                f"""update best_score set rating=0 where song_id not in ({','.join(['?']*len(songs))})""",
                songs,
            )
            ...
            for i in x:
                for j in range(0, 5):
                    ...
                    c.execute(
                        """select user_id, score, shiny_perfect_count, perfect_count, near_count, miss_count from best_score where song_id=:a and difficulty=:b""",
                        {"a": i[0], "b": j},
                    )
                    y = c.fetchall()
                    values = []
            ...
            for i in users:
                ...
                user_id = i[0]
                c.execute(
                    """select r_index, song_id, difficulty, score from recent30 where user_id = ?""",
                    (user_id,),
                )
                for j in c.fetchall():
...
class SaveUpdateScore(BaseOperation):
    ...
    def _one_user_update(self):
        with Connect() as c:
            ...
            clear_state = {
                f'{i["song_id"]}{i["difficulty"]}': i["clear_type"]
            ...
            c.execute(
                f"""select song_id, rating_pst, rating_prs, rating_ftr, rating_byn from chart where song_id in ({','.join(['?']*len(song_id))})""",
                song_id,
            )
            x = c.fetchall()
            song_chart_const = {
            ...
            for i in save.scores_data:
                ...
                if i["song_id"] in song_chart_const:
                    defnum = song_chart_const[i["song_id"]][i["difficulty"]] / 10
                    rating = Score.calculate_rating(defnum, i["score"])
                    rating = max(rating, 0)

                    score_v2 = Score.calculate_score_v2(
                        defnum,
                        i["shiny_perfect_count"],
                        i["perfect_count"],
                        i["near_count"],
                        i["miss_count"],
                    )
    ...
    def _all_update(self):
        with Connect() as c:
            ...
            song_chart_const = {
                i[0]: [i[1], i[2], i[3], i[4]] for i in c.fetchall()
            }  # chart const * 10
...
class UnlockUserItem(BaseOperation):
    ...
    ALLOW_TYPES = ["single", "pack", "world_song", "course_banner", "world_unlock"]

    def __init__(
        self, user=None, method: str = "unlock", item_types: list = ["single", "pack"]
    ):
        self.user = user
    ...
    def set_params(
        ...
        **kwargs,
        ...
        if method in ["unlock", "lock"]:
            self.method = method
        if isinstance(item_types, list) and all(
            i in self.ALLOW_TYPES for i in item_types
        ):
    ...
    def _one_user_insert(self):
        with Connect() as c:
            c.execute(
                """select exists(select * from user where user_id = ?)""",
                (self.user.user_id,),
            )
            if not c.fetchone()[0]:
                raise NoData(
            ...
            c.execute(
                f"""select item_id, type from item where type in ({','.join(['?'] * len(self.item_types))})""",
                self.item_types,
            )
            sql_list = [(self.user.user_id, i[0], i[1]) for i in c.fetchall()]
            c.executemany(
    ...
    def _all_insert(self):
        with Connect() as c:
            ...
            y = c.fetchall()
            sql_list = [(i[0], j[0], j[1]) for i in x for j in y]
            c.executemany(
                """insert or ignore into user_item values (?, ?, ?, 1)""", sql_list
            )
    ...
    def _one_user_delete(self):
        with Connect() as c:
            c.execute(
                f"""delete from user_item where user_id = ? and type in ({','.join(['?'] * len(self.item_types))})""",
                (self.user.user_id, *self.item_types),
            )

    def _all_delete(self):
        with Connect() as c:
            c.execute(
                f"""delete from user_item where type in ({','.join(['?'] * len(self.item_types))})""",
                self.item_types,
            )
...
def _delete_one_table(c, table_name, user_id):
    c.execute(
        f"""insert into db_deleted.{table_name} select * from {table_name} where user_id = ?""",
        (user_id,),
    )
...
class DeleteOneUser(BaseOperation):
    ...
    def _data_save(self, c):
        c.execute(
            f"""insert into db_deleted.friend select * from friend where user_id_me = ? or user_id_other = ?""",
            (self.user.user_id, self.user.user_id),
        )
        c.execute(
            f"""delete from friend where user_id_me = ? or user_id_other = ?""",
            (self.user.user_id, self.user.user_id),
        )
    ...
    def _clear_login(self, c):
        c.execute("""delete from login where user_id = ?""", (self.user.user_id,))
        c.execute("""delete from api_login where user_id = ?""", (self.user.user_id,))
