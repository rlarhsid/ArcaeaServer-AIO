...
class DatabaseInit:
    def __init__(
        ...
        init_folder_path: str = Config.DATABASE_INIT_PATH,
    ) -> None:
    ...
    def table_init(self) -> None:
        ...
        self.c.execute(
            """insert into config values("version", :a);""",
    ...
    def character_init(self) -> None:
        ...
        for i in range(0, len(self.init_data.char)):
            ...
            self.c.execute(
                ...
                (
                    i,
                    self.init_data.char[i],
                    max_level,
                    self.init_data.frag1[i],
                    self.init_data.prog1[i],
                    self.init_data.overdrive1[i],
                    self.init_data.frag20[i],
                    self.init_data.prog20[i],
                    self.init_data.overdrive20[i],
                    self.init_data.frag30[i],
                    self.init_data.prog30[i],
                    self.init_data.overdrive30[i],
                    self.init_data.skill_id[i],
                    self.init_data.skill_unlock_level[i],
                    skill_requires_uncap,
                    self.init_data.skill_id_uncap[i],
                    self.init_data.char_type[i],
                    uncapable,
                ),
            )

        self.c.execute(
            """insert into character values(?,?,20,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0)""",
            (
                99,
                "shirahime",
                38,
        ...
        for i in self.init_data.char_core:
            self.c.executemany(
                """insert into char_item values(?,?,'core',?)""",
    ...
    def item_init(self) -> None:
        """初始化物品信息"""
        self.c.executemany(
            """insert into item values(?,"core",1)""",
            [(i,) for i in self.init_data.cores],
        )
        self.c.executemany(
            """insert into item values(?,"world_song",1)""",
            [(i,) for i in self.init_data.world_songs],
        )
        self.c.executemany(
            """insert into item values(?,"world_unlock",1)""",
            [(i,) for i in self.init_data.world_unlocks],
        )
        self.c.executemany(
            """insert into item values(?,"course_banner",1)""",
            [(i,) for i in self.init_data.course_banners],
        )

        self.c.execute(
            """insert into item values(?,?,?)""", ("fragment", "fragment", 1)
        )
        self.c.execute("""insert into item values(?,?,?)""", ("memory", "memory", 1))
        self.c.execute(
            """insert into item values(?,?,?)""", ("anni5tix", "anni5tix", 1)
        )
        self.c.execute(
            """insert into item values(?,?,?)""", ("pick_ticket", "pick_ticket", 1)
        )
        ...
        self.c.execute(
            """insert into item values(?,?,?)""",  # 新手任务奖励曲
    ...
    def role_power_init(self) -> None:
        ...
        self.c.executemany(
            ...
            [
                (self.init_data.role[i], self.init_data.role_caption[i])
                for i in range(len(self.init_data.role))
            ],
        )
        ...
        self.c.executemany(
            ...
            [
                (self.init_data.power[i], self.init_data.power_caption[i])
                for i in range(len(self.init_data.power))
            ],
        )
    ...
    def admin_init(self) -> None:
        ...
        self.c.execute("""insert into user_role values(?, "admin")""", (x.user_id,))
...
class LogDatabaseInit:
    def __init__(
        self,
        db_path: str = Config.SQLITE_LOG_DATABASE_PATH,
        init_folder_path: str = Config.DATABASE_INIT_PATH,
    ) -> None:
        self.db_path = db_path
        self.init_folder_path = init_folder_path
    ...
    def table_init(self) -> None:
        ...
        with open(self.sql_path, "r") as f:
            self.c.executescript(f.read())
        self.c.execute(
            """insert into cache values("version", :a, -1);""",
            {"a": ARCAEA_LOG_DATBASE_VERSION},
        )
...
class FileChecker:
    ...
    def _check_update_database_log(self) -> bool:
        if not self.check_file(Config.SQLITE_LOG_DATABASE_PATH):
            ...
            with Connect(Config.SQLITE_LOG_DATABASE_PATH) as c:
                try:
                    x = c.execute(
                        '''select value from cache where key="version"'''
                    ).fetchone()
                except:
                    x = None
    ...
    def _check_update_database_main(
        self, db_path=Config.SQLITE_DATABASE_PATH, init_class=DatabaseInit
    ) -> bool:
        if not self.check_file(db_path):
            ...
            if not x or x[0] != ARCAEA_DATABASE_VERSION:
                ...
                try:
                    ...
                    backup_path = try_rename(
                        db_path,
                        os.path.join(
                            Config.SQLITE_DATABASE_BACKUP_FOLDER_PATH,
                            os.path.split(db_path)[-1] + ".bak",
                        ),
                    )
                    ...
                    self.logger.info(f"Success to update the file `{db_path}`.")

                except Exception as e:
                    self.logger.error(format_exc())
                    self.logger.warning(f"Fail to update the file `{db_path}`.")

        return True

    def check_update_database(self) -> bool:
        return (
            self._check_update_database_main()
            and self._check_update_database_log()
            and self._check_update_database_main(
                Config.SQLITE_DATABASE_DELETED_PATH, DeletedDatabaseInit
            )
        )

    @staticmethod
    def update_database(
        old_path: str, new_path: str = Config.SQLITE_DATABASE_PATH
    ) -> None:
        """更新数据库，并删除旧文件"""
    ...
    def check_before_run(self) -> bool:
        ...
        return (
            self.check_song_file()
            and self.check_content_bundle()
            and self.check_update_database()
            and self.check_world_map()
        )
