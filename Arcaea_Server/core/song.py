...
class Chart:
    ...
    def to_dict(self) -> dict:
        return {"difficulty": self.difficulty, "chart_const": self.chart_const}
    ...
    def select(self) -> None:
        self.c.execute(
            """select rating_pst, rating_prs, rating_ftr, rating_byn, rating_etr from chart where song_id=:a""",
            {"a": self.song_id},
        )
        x = self.c.fetchone()
        if x is None:
...
class Song:
    ...
    def from_list(self, x: list) -> "Song":
        ...
        self.name = x[1]
        self.charts = [
            Chart(self.c, self.song_id, 0),
            Chart(self.c, self.song_id, 1),
            Chart(self.c, self.song_id, 2),
            Chart(self.c, self.song_id, 3),
            Chart(self.c, self.song_id, 4),
        ]
    ...
    def from_dict(self, d: dict) -> "Song":
        ...
        for chart in d["charts"]:
            self.charts[chart["difficulty"]].defnum = round(chart["chart_const"] * 10)
        return self

    def delete(self) -> None:
        self.c.execute("""delete from chart where song_id=?""", (self.song_id,))

    def update(self) -> None:
        """全部更新"""
        self.c.execute(
            """update chart set name=?, rating_pst=?, rating_prs=?, rating_ftr=?, rating_byn=?, rating_etr=? where song_id=?""",
            (
                self.name,
                self.charts[0].defnum,
                self.charts[1].defnum,
                self.charts[2].defnum,
                self.charts[3].defnum,
                self.charts[4].defnum,
                self.song_id,
            ),
        )
    ...
    def select(self, song_id: str = None) -> "Song":
        ...
        self.c.execute("""select * from chart where song_id=:a""", {"a": self.song_id})
        x = self.c.fetchone()
        if x is None:
