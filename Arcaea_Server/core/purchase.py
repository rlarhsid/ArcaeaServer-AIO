...
class Purchase(CollectionItemMixin):
    ...
    def to_dict(self, has_items: bool = True, show_real_price: bool = True) -> dict:
        ...
        if self.discount_from > 0 and self.discount_to > 0:
            ...
            if not show_real_price or (
                self.discount_reason in ("anni5tix", "pick_ticket") and price == 0
            ):
    ...
    def select(self, purchase_name: str = None) -> "Purchase":
        ...
        self.c.execute(
            """select * from purchase where purchase_name=:name""",
            {"name": self.purchase_name},
        )
        x = self.c.fetchone()
        if not x:
            raise NoData(f"Purchase `{self.purchase_name}` does not exist.", 501)
    ...
    def select_items(self) -> None:
        ...
        self.c.execute(
            """select item_id, type, amount from purchase_item where purchase_name=:a""",
            {"a": self.purchase_name},
        )
        x = self.c.fetchall()
        # if not x:
        #     raise NoData(
        ...
        for i in x:
            if i[0] == self.purchase_name:
                ...
                t = ItemFactory.from_dict(
                    {"item_id": i[0], "type": i[1], "amount": i[2] if i[2] else 1},
                    self.c,
                )
            else:
                self.items.append(
    ...
    def buy(self) -> None:
        ...
        if self.user.ticket < price_used:
            raise TicketNotEnough("The user does not have enough memories.", -6)

        if not (
            self.orig_price == 0
            or self.price == 0
            and self.discount_from <= int(time() * 1000) <= self.discount_to
        ):
            if price_used == 0:
    ...
    def update(self) -> None:
        """更新purchase表"""
        self.c.execute(
            """update purchase set price=:a, orig_price=:b, discount_from=:c, discount_to=:d, discount_reason=:e where purchase_name=:f""",
            {
                "a": self.price,
                "b": self.orig_price,
                "c": self.discount_from,
                "d": self.discount_to,
                "e": self.discount_reason,
                "f": self.purchase_name,
            },
        )
...
class PurchaseList:
    ...
    def select_from_type(self, item_type: str) -> "PurchaseList":
        self.c.execute(
            """select purchase_name from purchase_item where type = :a""",
            {"a": item_type},
        )
        x = self.c.fetchall()
        if not x:
