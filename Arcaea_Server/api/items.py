...
def items_get(data, user):
    """查询全物品信息"""
    with Connect() as c:
        query = Query(["item_id", "type"], ["item_id"], ["item_id"]).from_dict(data)
        x = Sql(c).select("item", query=query)
        r: "list[Item]" = []
        for i in x:
            r.append(
                ItemFactory.from_dict(
                    {"item_id": i[0], "type": i[1], "is_available": i[2] == 1}
                )
            )
        ...
        return success_return(
            [x.to_dict(has_is_available=True, has_amount=False) for x in r]
        )
...
@request_json_handle(
    request, required_keys=["item_id", "type"], optional_keys=["is_available"]
)
@api_try
def items_post(data, user):
    """新增物品"""
    if data["type"] not in ALLOW_ITEM_TYPE:
        raise InputError(f'Invalid item type: `{data["type"]}`', api_error_code=-120)
    with Connect() as c:
        item = ItemFactory.from_dict(data, c=c)
        if item.select_exists():
            raise DataExist(
                f"Item `{item.item_type}`: `{item.item_id}` already exists",
                api_error_code=-122,
            )
        item.insert()
...
def items_item_delete(user, item_type, item_id):
    ...
    if item_type not in ALLOW_ITEM_TYPE:
        raise InputError(f"Invalid item type: `{item_type}`", api_error_code=-120)
    with Connect() as c:
        item = ItemFactory.from_dict({"item_id": item_id, "type": item_type}, c=c)
...
def items_item_put(data, user, item_type, item_id):
    ...
    if not isinstance(data["is_available"], bool):
        raise InputError("`is_available` must be a boolean", api_error_code=-101)
    with Connect() as c:
