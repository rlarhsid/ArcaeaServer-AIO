...
def users_post(data, user):
    ...
    with Connect() as c:
        ...
        return success_return(
            {"user_id": new_user.user_id, "user_code": new_user.user_code}
        )
...
@request_json_handle(
    request,
    optional_keys=["name", "password", "user_code", "ticket", "email"],
    must_change=True,
)
@api_try
...
def users_user_b30_get(user, user_id):
    ...
    with Connect() as c:
        ...
        x.select_from_user()
        if not x.scores:
            raise NoData(f"No best30 data of user `{user_id}`", api_error_code=-3)
        x.select_song_name()
        r = x.to_dict_list()
        rating_sum = sum(i.rating for i in x.scores)
        return success_return(
            {"user_id": user_id, "b30_ptt": rating_sum / 30, "data": r}
        )
...
def users_user_best_get(data, user, user_id):
    ...
    with Connect() as c:
        ...
        x.select_from_user()
        if not x.scores:
            raise NoData(f"No best score data of user `{user_id}`", api_error_code=-3)
        r = x.to_dict_list()
...
def users_user_r30_get(user, user_id):
    ...
    with Connect() as c:
        ...
        return success_return(
            {
                "user_id": user_id,
                "r10_ptt": p.recent_10 / 10,
                "data": p.recent_30_to_dict_list(),
            }
        )
        ...
        return success_return(
            {
                ...
                "powers": [i.power_id for i in user.role.powers],
            }
        )
    # 查别人需要select权限
        ...
        return success_return(
            {
                "user_id": x.user_id,
                "role": x.role.role_id,
                "powers": [i.power_id for i in x.role.powers],
            }
        )
...
@role_required(request, ["select", "select_me"])
@request_json_handle(
    request, optional_keys=["start_timestamp", "end_timestamp", "duration"]
)
@api_try
def users_user_rating_get(data, user, user_id):
    ...
    with Connect(Config.SQLITE_LOG_DATABASE_PATH) as c:
        ...
        return success_return(
            {
                "user_id": user_id,
                "data": [{"time": i[0], "rating_ptt": i[1]} for i in r],
            }
        )
