...
def characters_character_put(data, user, character_id: int):
    ...
    if (
        "skill_id" in data
        and data["skill_id"] != ""
        and data["skill_id"] not in Constant.SKILL_IDS
    ) or (
        "skill_id_uncap" in data
        and data["skill_id_uncap"] != ""
        and data["skill_id_uncap"] not in Constant.SKILL_IDS
    ):
...
def characters_character_cores_patch(data, user, character_id: int):
    ...
    with Connect() as c:
        ...
        ch.update_items([ItemFactory.from_dict(x, c=c) for x in updates])
