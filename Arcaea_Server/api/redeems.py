...
def redeems_post(data, user):
    ...
    with Connect() as c:
        ...
        if r.select_exists():
            raise DataExist(f"redeem `{r.code}` already exists")
        r.insert_all()
...
def redeems_redeem_items_patch(data, user, code: str):
    ...
    with Connect() as c:
        ...
        r.code = code
        r.select_items()
        r.remove_items([ItemFactory.from_dict(x, c=c) for x in data.get("remove", [])])
        r.add_items([ItemFactory.from_dict(x, c=c) for x in data.get("create", [])])
