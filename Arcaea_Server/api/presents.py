...
def presents_get(data, user):
    ...
    with Connect() as c:
        query = Query(
            ["present_id"], ["present_id", "description"], ["present_id", "expire_ts"]
        ).from_dict(data)
...
@request_json_handle(
    request,
    required_keys=["present_id", "description", "expire_ts"],
    optional_keys=["items"],
)
@api_try
def presents_post(data, user):
    ...
    with Connect() as c:
        ...
        if p.select_exists():
            raise DataExist(f"Present `{p.present_id}` already exists")
        p.insert_all()
...
@request_json_handle(
    request, optional_keys=["description", "expire_ts"], must_change=True
)
@api_try
