...
def purchases_get(data, user):
    ...
    with Connect() as c:
        ...
        return success_return(
            [x.to_dict(has_items=False, show_real_price=False) for x in r]
        )
...
@request_json_handle(
    request,
    required_keys=["purchase_name", "orig_price"],
    optional_keys=["price", "discount_from", "discount_to", "discount_reason", "items"],
)
@api_try
def purchases_post(data, user):
    ...
    with Connect() as c:
        ...
        if purchase.select_exists():
            raise DataExist(f"Purchase `{purchase.purchase_name}` already exists")
        purchase.insert_all()
        return success_return(
            purchase.to_dict(has_items="items" in data, show_real_price=False)
        )
...
def purchases_purchase_get(user, purchase_name: str):
    ...
    with Connect() as c:
        return success_return(
            Purchase(c).select(purchase_name).to_dict(show_real_price=False)
        )
...
def purchases_purchase_items_patch(data, user, purchase_name: str):
    ...
    with Connect() as c:
        ...
        p.purchase_name = purchase_name
        p.select_items()
        p.remove_items([ItemFactory.from_dict(x, c=c) for x in data.get("remove", [])])
        p.add_items([ItemFactory.from_dict(x, c=c) for x in data.get("create", [])])
