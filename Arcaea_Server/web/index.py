    ...
    if request.method == "POST":
        ...
        if name or user_code:
            with Connect() as c:
                if user_code:
                    c.execute(
                        """select user_id from user where user_code=:a""",
                        {"a": user_code},
                    )
                else:
                    c.execute("""select user_id from user where name=:a""", {"a": name})
    ...
    with Connect() as c:
        ...
        if x:
            ...
            for i in x:
                ...
                if i[20]:
                    time_played = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(int(i[20]) // 1000)
                    )
                if i[2] == "":
                    ban_flag = True
                else:
...
def single_chart_top():
    ...
    if request.method == "POST":
        ...
        with Connect() as c:
            song_name = "%" + song_name + "%"
            c.execute(
                """select song_id, name from chart where song_id like :a limit 1""",
                {"a": song_name},
            )
            x = c.fetchone()
            ...
            if x:
                ...
                for i in posts:
                    i["time_played"] = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(i["time_played"])
                    )
            else:
                error = "查询为空 No song."
...
def update_database():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            and file.filename in ["arcaea_database.db"]
        ):
            filename = "old_" + secure_filename(file.filename)
            file.save(os.path.join(UPLOAD_FOLDER, filename))
            flash("上传成功 Success upload.")
            try:
                FileChecker.update_database(os.path.join(UPLOAD_FOLDER, filename))
...
def add_song():
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (0,):
            c.execute(
                """insert into chart values(:a,:b,:c,:d,:e,:f,:g)""",
                {
                    "a": song_id,
                    "b": name_en,
                    "c": rating_pst,
...
def edit_char():
    ...
    with Connect() as c:
        c.execute(
            """select exists(select * from character where character_id=:a)""",
            {"a": character_id},
        )
        if c.fetchone() == (1,):
    ...
    with Connect() as c:
        ...
        if "name" not in request.form and "user_code" not in request.form:
            ...
            if not ticket:
                ...
                c.execute(sql, sql_dict)
                flash(
                    "全部用户信息修改成功 Successfully edit all the users' information."
                )
        ...
        if name or user_code:
            ...
            if user_code:
                c.execute(
                    """select user_id from user where user_code=:a""", {"a": user_code}
                )
            else:
                c.execute("""select user_id from user where name=:a""", {"a": name})
            ...
            if user_id:
                ...
                if not ticket:
                    ...
                    sql = (
                        """update user set ticket = :ticket where user_id = :user_id"""
                    )
    ...
    with Connect() as c:
        ...
        if "name" not in request.form and "user_code" not in request.form:
            ...
            if method == "0":
                ...
                c.execute("""delete from user_item where type in ('pack', 'single')""")

            flash(
                "全部用户购买信息修改成功 Successfully edit all the users' purchase information."
            )
        ...
        if name or user_code:
            ...
            if user_id:
                ...
                if method == "0":
                    ...
                    c.execute(
                        """delete from user_item where type in ('pack', 'single') and user_id = :user_id""",
                        {"user_id": user_id},
                    )
                flash(
                    "用户购买信息修改成功 Successfully edit the user purchase information."
                )
...
def change_item():
    ...
    if request.method == "POST":
        ...
        with Connect() as c:
            c.execute(
                """select exists(select * from item where item_id=:a and type=:b)""",
...
def change_item_delete():
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (1,):
            c.execute(
                """delete from item where item_id=? and type=?""", (item_id, item_type)
            )
...
def change_purchase():
    ...
    if request.method == "POST":
        try:
            ...
            if discount_from:
                discount_from = (
                    int(time.mktime(time.strptime(discount_from, "%Y-%m-%dT%H:%M")))
                    * 1000
                )
            else:
                discount_from = -1
            if discount_to:
                discount_to = (
                    int(time.mktime(time.strptime(discount_to, "%Y-%m-%dT%H:%M")))
                    * 1000
                )
            else:
                discount_to = -1
        ...
        with Connect() as c:
            c.execute(
                """select exists(select * from purchase where purchase_name=:a)""",
                {"a": purchase_name},
            )
            if c.fetchone() == (0,):
                c.execute(
...
def change_purchase_delete():
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (1,):
            c.execute(
                """delete from purchase where purchase_name=?""", (purchase_name,)
            )
            c.execute(
                """delete from purchase_item where purchase_name=?""", (purchase_name,)
            )
...
def change_purchase_item():
    ...
    if request.method == "POST":
        ...
        with Connect() as c:
            c.execute(
                """select exists(select * from purchase_item where purchase_name=? and item_id=? and type=?)""",
                (purchase_name, item_id, item_type),
            )
            if c.fetchone() == (0,):
                c.execute(
                    """select exists(select * from purchase where purchase_name=?)""",
                    (purchase_name,),
                )
                if c.fetchone() == (1,):
                    c.execute(
                        """select exists(select * from item where item_id=? and type=?)""",
                        (item_id, item_type),
                    )
                    if c.fetchone() == (1,):
                        c.execute(
                        ...
                        flash(
                            """购买项目的物品添加成功 Successfully add the purchase's item."""
                        )
                    else:
                        error = """物品不存在 The item does not exist."""
                else:
...
def change_purchase_item_delete():
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (1,):
            c.execute(
                """delete from purchase_item where purchase_name=? and item_id=? and type=?""",
                (purchase_name, item_id, item_type),
            )
...
def all_present():
    ...
    with Connect() as c:
        ...
        if x:
            ...
            for i in x:
                items = []
                c.execute("""select * from present_item where present_id=?""", (i[0],))
                y = c.fetchall()
                for j in y:
                    if j is not None:
                ...
                posts.append(
                    {
                        ...
                        "expire_ts": time.strftime(
                            "%Y-%m-%d %H:%M:%S", time.localtime(int(i[1]) // 1000)
                        ),
                        "items": items,
                        "description": i[2],
                    }
...
def add_present():
    ...
    try:
        if item_amount:
            ...
            item_amount = 1
        expire_ts = int(time.mktime(time.strptime(expire_ts, "%Y-%m-%dT%H:%M"))) * 1000
    except:
...
def all_redeem():
    ...
    with Connect() as c:
        ...
        if x:
            ...
            for i in x:
                items = []
                c.execute("""select * from redeem_item where code=?""", (i[0],))
                y = c.fetchall()
                for j in y:
                    if j is not None:
                        items.append({"item_id": j[1], "type": j[2], "amount": j[3]})

                posts.append({"code": i[0], "items": items, "type": i[1]})
        else:
...
def one_redeem(code):
    ...
    with Connect() as c:
        c.execute(
            """select user_id, name, user_code from user where user_id in (select user_id from user_redeem where code=:a)""",
            {"a": code},
        )
        x = c.fetchall()
        if x:
            posts = []
            for i in x:
                posts.append({"user_id": i[0], "name": i[1], "user_code": i[2]})
        else:
            error = "没有数据 No data."
    ...
    with Connect() as c:
        if name or user_code:
            ...
            if user_id:
                user_id = user_id[0]
                c.execute(
                    """update user set rating_ptt=0, song_id='', difficulty=0, score=0, shiny_perfect_count=0, perfect_count=0, near_count=0, miss_count=0, health=0, time_played=0, rating=0, world_rank_score=0 where user_id=:a""",
                    {"a": user_id},
                )
                c.connection.commit()
