...
def update_user_char(c):
    ...
    if x and y:
        for j in y:
            for i in x:
                c.execute(
                    """delete from user_char_full where user_id=:a and character_id=:b""",
                    {"a": j[0], "b": i[0]},
                )
...
def get_all_item():
    ...
    with Connect() as c:
        ...
        if x:
            for i in x:
                re.append({"item_id": i[0], "type": i[1], "is_available": int2b(i[2])})
...
def add_one_present(
    ...
    with Connect() as c:
        c.execute(
            """select exists(select * from present where present_id=:a)""",
            {"a": present_id},
        )
        if c.fetchone() == (0,):
            c.execute(
            ...
            if c.fetchone() == (1,):
                c.execute(
                    """insert into present values(:a,:b,:c)""",
                    {"a": present_id, "b": expire_ts, "c": description},
                )
                c.execute(
                    """insert into present_item values(?,?,?,?)""",
...
def delete_one_present(present_id):
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (1,):
            c.execute(
                """delete from present where present_id = :a""", {"a": present_id}
            )
            c.execute(
                """delete from user_present where present_id =:a""", {"a": present_id}
            )
            c.execute(
                """delete from present_item where present_id =:a""", {"a": present_id}
            )
...
def deliver_one_user_present(c, present_id, user_id):
    # 为指定玩家添加奖励，重复添加不会提示
    c.execute(
        """select exists(select * from user_present where user_id=:a and present_id=:b)""",
        {"a": user_id, "b": present_id},
    )
    if c.fetchone() == (0,):
        c.execute(
            """insert into user_present values(:a,:b)""",
            {"a": user_id, "b": present_id},
        )
    return
...
def deliver_all_user_present(c, present_id):
    ...
    x = c.fetchall()
    if x:
        c.execute("""delete from user_present where present_id=:b""", {"b": present_id})
        for i in x:
            c.execute(
                """insert into user_present values(:a,:b)""",
                {"a": i[0], "b": present_id},
...
def add_one_redeem(code, redeem_type, item_id, item_type, item_amount):
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (0,):
            ...
            if c.fetchone() == (1,):
                c.execute(
                    """insert into redeem values(:a,:c)""",
                    {"a": code, "c": redeem_type},
                )
                c.execute(
                    """insert into redeem_item values(?,?,?,?)""",
...
def delete_one_redeem(code):
    ...
    with Connect() as c:
        ...
        if c.fetchone() == (1,):
            ...
            c.execute("""delete from redeem_item where code =:a""", {"a": code})
            message = "删除成功 Successfully delete it."
        else:
...
def change_userpwd(c, user_id, password):
    ...
    c.execute(
        """update user set password =:a where user_id=:b""",
...
def ban_one_user(c, user_id):
    # 封禁用户
    c.execute("""update user set password = '' where user_id=:a""", {"a": user_id})
