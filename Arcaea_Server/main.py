        ...
        try:
            ...
            if Config.DOWNLOAD_USE_NGINX_X_ACCEL_REDIRECT:
                ...
                response.headers["X-Accel-Redirect"] = (
                    Config.NGINX_X_ACCEL_REDIRECT_PREFIX + file_path
                )
                return response
    ...
    with Connect(in_memory=True) as c_m:
        try:
            ...
            if Config.DOWNLOAD_USE_NGINX_X_ACCEL_REDIRECT:
                ...
                response.headers["Content-Type"] = "application/octet-stream"
                response.headers["X-Accel-Redirect"] = (
                    Config.BUNDLE_NGINX_X_ACCEL_REDIRECT_PREFIX + file_path
                )
                return response
            return send_from_directory(
                Constant.CONTENT_BUNDLE_FOLDER_PATH,
                file_path,
                as_attachment=True,
                conditional=True,
            )
...
def tcp_server_run():
    if Config.DEPLOY_MODE == "gevent":
        ...
        if Config.SSL_CERT and Config.SSL_KEY:
            app.run(
                Config.HOST, Config.PORT, ssl_context=(Config.SSL_CERT, Config.SSL_KEY)
            )
        else:
...
def main():
    log_dict = {
        ...
        "handlers": {
            ...
            "error_file": generate_log_file_dict(
                "ERROR", f"{Config.LOG_FOLDER_PATH}/error.log"
            ),
        },
        "formatters": {
            "default": {
        ...
        process = [
            Process(
                ...
                args=(
                    ...
                    int(Config.LINKPLAY_UDP_PORT),
                    int(Config.LINKPLAY_TCP_PORT),
                ),
            )
        ]
